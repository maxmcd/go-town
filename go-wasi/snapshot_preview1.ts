import { relative, resolve } from "https://deno.land/std@0.93.0/path/mod.ts";
...
function syscall<T extends CallableFunction>(target: T) {
  return function (...args: unknown[]) {
    try {
      const retval = target(...args);
      if (retval) {
        console.log(target.toString(), retval, args);
      }
      return retval;
    } catch (err) {
...
export default class Context {
  ...
  constructor(options: ContextOptions) {
    ...
    this.exports = {
      args_get: syscall(
        (argvOffset: number, argvBufferOffset: number): number => {
      ...
      args_sizes_get: syscall(
        (argcOffset: number, argvBufferSizeOffset: number): number => {
          const args = this.#args;
          const textEncoder = new TextEncoder();
          const memoryView = new DataView(this.#memory.buffer);
      ...
      environ_get: syscall(
        (environOffset: number, environBufferOffset: number): number => {
      ...
      environ_sizes_get: syscall(
        (environcOffset: number, environBufferSizeOffset: number): number => {
          const entries = Object.entries(this.#env);
          const textEncoder = new TextEncoder();
          const memoryView = new DataView(this.#memory.buffer);
      ...
      clock_res_get: syscall((id: number, resolutionOffset: number): number => {
        const memoryView = new DataView(this.#memory.buffer);
      ...
      clock_time_get: syscall(
        (id: number, precision: bigint, timeOffset: number): number => {
          const memoryView = new DataView(this.#memory.buffer);
      ...
      fd_allocate: syscall(
        (_fd: number, _offset: bigint, _length: bigint): number => {
          return ERRNO_NOSYS;
        }
      ),
      ...
      fd_fdstat_get: syscall((fd: number, offset: number): number => {
        const entry = this.#fds[fd];
        if (!entry) {
          return ERRNO_BADF;
        }
      ...
      fd_filestat_get: syscall((fd: number, offset: number): number => {
        const entry = this.#fds[fd];
        if (!entry) {
          return ERRNO_BADF;
        }
      ...
      fd_filestat_set_times: syscall(
        (fd: number, atim: bigint, mtim: bigint, flags: number): number => {
          const entry = this.#fds[fd];
          if (!entry) {
            return ERRNO_BADF;
          }
      ...
      fd_prestat_get: syscall((fd: number, prestatOffset: number): number => {
        const entry = this.#fds[fd];
        if (!entry) {
          return ERRNO_BADF;
        }
      ...
      fd_prestat_dir_name: syscall(
        (fd: number, pathOffset: number, pathLength: number): number => {
          const entry = this.#fds[fd];
          if (!entry) {
            return ERRNO_BADF;
          }
      ...
      fd_tell: syscall((fd: number, offsetOffset: number): number => {
        const entry = this.#fds[fd];
        if (!entry) {
          return ERRNO_BADF;
        }
      ...
      random_get: syscall(
        (bufferOffset: number, bufferLength: number): number => {
      ...
      sock_shutdown: syscall((_fd: number, _how: number): number => {
        return ERRNO_NOSYS;
      }),
    };
