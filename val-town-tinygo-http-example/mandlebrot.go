...
func renderPresetImage() []byte {
	...
	colors := s.InterpolateColors(&s.ColorPalette, s.ColorStep)
	return s.Render(s.MaxIteration, colors, done)
}
...
func (s *Service) Render(maxIteration int, colors []color.RGBA, done chan struct{}) []byte {
